...
class threeDeeObject():
    ...
    def show3D(self):
        ...
        imgMaskSequence = fnmatch.filter(listdir(self.dir), '*_maskCurves.png') #mascaras de las imgs:
        ...
        if not len(imgMaskSequence) == 0:
            ...
            for image in imgMaskSequence:
                ...
                img = io.imread(self.dir + '/' + image) #leemos la img correspondiente a la mascara que estamos analizando
                ...
                if not np.sum(img)==0: #si la mascara de la img no contiene ninguna curva (esta a 0/negro entera)...
                    ...
                    dis = self.euclDistanceFromCenter(img) #calculamos la matriz de distancias a partir de la mascara creada
